...
namespace Wintellect.Sterling
{
    ...
    public class MemoryDriver : BaseDriver 
    {
        ...
        ///     Objects
        /// </summary>
        private readonly Dictionary<GenericTuple<string,int>, byte[]> _objectCache = new Dictionary<GenericTuple<string,int>, byte[]>();
        ...
        /// <param name="indexName">The name of the index</param>
        /// <param name="indexMap">The index map</param>        
        public override void SerializeIndex<TKey, TIndex1, TIndex2>(Type type, string indexName, Dictionary<TKey, GenericTuple<TIndex1, TIndex2>> indexMap)
        {
            lock (((ICollection)_indexCache).SyncRoot)
            {
                if (!_indexCache.ContainsKey(type))
        ...
        /// <param name="indexName">The name of the index</param>        
        /// <returns>The index map</returns>        
        public override Dictionary<TKey, GenericTuple<TIndex1, TIndex2>> DeserializeIndex<TKey, TIndex1, TIndex2>(Type type, string indexName)
        {
            lock (((ICollection)_indexCache).SyncRoot)
            {
                if (!_indexCache.ContainsKey(type))
                ...
                return indexCache[indexName] as Dictionary<TKey, GenericTuple<TIndex1, TIndex2>>;
            }
        }
        ...
        {
            var key = SterlingTuple.Create(type.FullName, keyIndex);
