...
namespace Wintellect.Sterling.Indexes
{
    ...
    /// </summary>
    internal class IndexCollection<T, TIndex1, TIndex2, TKey> : IndexCollection<T, GenericTuple<TIndex1, TIndex2>, TKey>
        where T : class, new()
    {
        /// <summary>
        ...
        public IndexCollection(string name, ISterlingDriver driver,
                               Func<T, GenericTuple<TIndex1, TIndex2>> indexer, Func<TKey, T> resolver) 
        ...
        protected override void DeserializeIndexes()
        {
            ...
            foreach (var index in Driver.DeserializeIndex<TKey, TIndex1, TIndex2>(typeof(T), Name) ?? new Dictionary<TKey, GenericTuple<TIndex1, TIndex2>>())
            {
        ...
        {
            var dictionary = IndexList.ToDictionary(item => item.Key, item => SterlingTuple.Create(item.Index.Item1, item.Index.Item2));
