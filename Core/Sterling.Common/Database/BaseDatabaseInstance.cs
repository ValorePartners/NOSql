...
namespace Wintellect.Sterling.Database
{
    ...
    public abstract class BaseDatabaseInstance : ISterlingDatabaseInstance
    {
        ...
        /// <param name="indexName">The name of the index</param>
        /// <returns>The indexed items</returns>
        public List<TableIndex<T, GenericTuple<TIndex1, TIndex2>, TKey>> Query<T, TIndex1, TIndex2, TKey>(string indexName)
            where T : class, new()
        {
            if (string.IsNullOrEmpty(indexName))
            {
            ...
            return new List<TableIndex<T, GenericTuple<TIndex1, TIndex2>, TKey>>(collection.Query);
        }
