...
namespace Wintellect.Sterling.Database
{
    ...
    internal class SterlingDatabase : ISterlingDatabase
    {
        ...
        /// </summary>
        private readonly Dictionary<string,GenericTuple<Type,ISterlingDatabaseInstance>> _databases = new Dictionary<string,GenericTuple<Type,ISterlingDatabaseInstance>>();
        ...
        public void Restore<T>(BinaryReader reader) where T : BaseDatabaseInstance
        {
            ...
            foreach (var table in ((BaseDatabaseInstance) database).TableDefinitions)
            {
                ...
                foreach (var instance in from object key in keyDictionary.Keys select SterlingTuple.Create(key, database.Load(table1.Key, key)))
                {
        ...
        public ISterlingDatabaseInstance RegisterDatabase<T>(ISterlingDriver driver) where T : BaseDatabaseInstance
        {
            ...
            _databases.Add(database.Name, new GenericTuple<Type, ISterlingDatabaseInstance>(typeof(T), database));
            return database;
        }
